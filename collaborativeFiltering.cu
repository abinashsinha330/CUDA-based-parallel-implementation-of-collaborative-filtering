...
//     CopyToDeviceMatrix(d_out, out);
        ...
        //cudaMalloc((void**)&d_out.elements, 4*6*sizeof(float));
        transpose<<<transGrid, transBlock>>>(d_transpose, d_ratings, COLS, ROWS);

        //transposeMatrix();
        //cudaMemcpy(d_transpose,transpose, ROWS * COLS * sizeof(float), cudaMemcpyHostToDevice);
        cudaEventRecord(stop);
	...
	for(int i=15;i<34;i++)
        {
                for(int j=15;j<34;j++)
                {
                        printf("%f\t:%f\n",global_sim[i*ROWS+j],sim[i*ROWS+j]);
                }
        }
